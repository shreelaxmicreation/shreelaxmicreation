...
export default async function AboutPage() {
  const siteSettings = await client.fetch<SanitySiteSettings | null>(siteSettingsQuery)

  const aboutHeroBg = siteSettings?.aboutHeroImage
    ? urlFor(siteSettings.aboutHeroImage).width(2000).quality(80).format('webp').url()
    : '/images/about.jpg'

  const factoryImg = siteSettings?.factoryImage
    ? urlFor(siteSettings.factoryImage).width(1200).quality(80).format('webp').url()
    : '/images/factory-interior.png'
